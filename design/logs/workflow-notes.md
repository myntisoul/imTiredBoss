...
на самом деле для этого изображения я использовал flux schnell, но результат мне так понравился, что даже когда я делал depth с lora'ми ps1-style и low-poly на flux.1 dev fp8, меня результат всё равно меньше понравился.
...
### Контекст
- **кадр**
	`01-w06-scene-happy-dream2`
- **Что в нём уже работало**
	атмосфера, расплывчатость, ощущение сна
- **Что нужно было исправить**
	по большей части ничего, просто протестировать outpaint. хотел расширить пейзаж и сделать героев меньше, будто они где-то вдалеке, чтобы показать насколько эта картина далека от реальности, в которой происходит сюжет.
### Что фиксировали
- **Модель**: `flux.1 dev fp8 fill`
- **Workflow**: `w07_outpaint`
- **Seed**: `919828922167906`
- **Основные параметры**:
	- cfg: 1.0
	- steps: 20
...
### Что меняли
- область расширения.
	- left: 400, right: 400, top: 0, bottom: 400
	- появились новые фрагменты пейзажа.
### Вывод
- **Что принимаем**: новый вариант
- **Почему**: кадр не испортился
- **Что делаем дальше**: оставляем так

## HW08 — ControlNet Depth / Canny

### Цель 1
- Какие будущие скрингены я готовлю: финальное столкновение виктора и валерии несс
- Что хочу контролировать: позы, контуры объектов
- Почему выбрал Canny: потому что я понял, как это делать 
### Использованный workflow 1
- Tool: local ComfyUI
- Workflow: `w08_canny`
- Model: flux.1 dev fp8 canny
- Control mode: canny
- Input image / sketch / reference: canny1.png 
### Цель 2
- Какие будущие скрингены я готовлю: кабинет оракула
- Что хочу контролировать: глубину
- Почему выбрал depth: просто надо попробовать 
### Использованный workflow 2
- Tool: local ComfyUI
- Workflow: `w08_depth`
- Model: flux.1 dev fp8 
- Control mode: depth
- Input image / sketch / reference: `depth map.png` 
### Результаты

| File                  | Mode  | Input           | Что сработало               | Что сломалось                                                                            | Решение  |
| --------------------- | ----- | --------------- | --------------------------- | ---------------------------------------------------------------------------------------- | -------- |
| 08-depth-scene-01.png | Depth | `depth map.png` | глубина                     | в целом всё неплохо (но `01-w04-location-oracle's-office` мне всё равно нравится больше) | selected |
| 08-canny-scene-01.png | Canny | `canny1.png`    | более менее сработали позы, | топор виктора выглядит странно, рядом с несс нет автомата                                | selected |

### Вывод
- Какой подход лучше сработал для моего проекта: canny (только потому что для depth у меня есть альтернатива в виде `01-w04-location-oracle's-office`, которая мне нравится больше)
- Что можно использовать для скрингена: `08-canny-scene-01`
- Что нужно доработать позже: топор виктора выглядит странно, автомата рядом с трупом несс нет, хотя я указывал его в промте.

## W09 — IP-Adapter / Flux 2 / Flux 2 Klein / Simple Upscale
* что показал преподаватель по IP-Adapter
	* сгенерировал картинку с лицом трампа
* какие ограничения есть у cloud.comfy.org
	* - одновременно можно запускать только ограниченное число workflow;
	- есть лимит времени на выполнение одного workflow (обычно 10–30 минут в зависимости от тарифа);
	- существует лимит GPU-часов / кредитов;
	- нельзя свободно устанавливать любые custom nodes;
	- нельзя полностью настраивать окружение вручную;
	- не все локальные workflow совместимы с cloud-версией;
	- загрузка собственных моделей и checkpoint’ов ограничена;
	- часть функций доступна только на платных тарифах;
	- нужен интернет, оффлайн-работа невозможна;
	- API и некоторые advanced-функции пока ограничены или находятся в beta.
* какие подходы могут быть полезны вашему проекту
	* думаю, upscale мне очень пригодится, т.к. почти все изображения я генерировал в 512х512 для экономии времени.
* какие 2 ассета вы потенциально хотите довести через upscale
	* самые мыльные сейчас это `01-w04-character-messiah` и `01-w04-character-protagonist`

| Подход                 | Что делает                                                                                                                                                            | Где полезен в моём проекте                                                                                                                                                                      | Ограничения                                                                                                                                                              | Нужен ли мне сейчас                                                                                                           |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| IP-Adapter             | Использует изображение-референс для переноса визуального стиля, палитры, атмосферы и общего вайба сцены. Может удерживать внешний вид персонажа между кадрами.        | Полезен для сохранения consistent атмосферы кабинета Оракула, внешности Виктора и low-poly PS1-эстетики между разными сценами. Можно использовать референсы освещения, материалов и композиции. | Есть риск слишком сильного копирования референса. Плохо контролирует точную позу и композицию. Не заменяет ControlNet. Требует аккуратного подбора изображений.          | Да. Особенно полезен для дальнейших катсцен и поддержания единого визуального стиля проекта.                                  |
| Flux 2 / image editing | Подход для редактирования изображений через img2img, inpaint, outpaint и multi-reference. Позволяет изменять существующее изображение вместо полной генерации с нуля. | Полезен для исправления кадров, замены одежды персонажей, изменения света, дорисовки окружения и улучшения сцен после генерации. Особенно важен для катсцен и исправления ошибок Flux.          | Некоторые workflow и модели могут быть недоступны в cloud.comfy.org. Требует больше VRAM и более сложной настройки. Multi-reference может давать нестабильный результат. | Да                                                                                                                            |
| Flux 2 Klein           | Облегчённый pipeline Flux для более быстрых локальных итераций. Использует diffusion_model, text_encoder и VAE в упрощённой конфигурации.                             | Подходит для быстрых тестов композиции, света и multi-prompt без долгого рендера. Может использоваться для черновых сцен и поиска baseline.                                                     | Более низкое качество по сравнению с полным Flux pipeline. Меньше гибкости. Не все advanced-функции доступны. Запуск не обязателен для проекта.                          | Частично. Может пригодиться для быстрых локальных тестов, но основной production pipeline проекта остаётся на Flux.1 dev fp8. |
| Simple upscale         | Увеличивает разрешение изображения и слегка улучшает детализацию. Не изменяет композицию сцены.                                                                       | Полезен для финальных кадров катсцен, презентации проекта и улучшения already-good изображений.                                                                                                 | Не исправляет ошибки композиции, анатомии, света или плохой генерации. Бесполезен для неудачных изображений. Может усиливать артефакты.                                  | Да, но только как финальный этап после основной генерации и image editing.                                                    |
Для дальнейшей работы над проектом наиболее полезными являются IP-Adapter и Flux image editing workflow. Они помогают удерживать единый визуальный стиль, consistent атмосферу и исправлять проблемы генерации без создания изображения с нуля.
ControlNet остаётся основным способом контроля композиции и поз, а IP-Adapter — способом удержания атмосферы, палитры и дизайна персонажей.
Flux 2 Klein может использоваться для быстрых тестов и черновых итераций, но не является основным production workflow проекта.
Simple upscale будет использоваться только для финальной подготовки удачных изображений, так как он не исправляет ошибки генерации.
## Upscale candidates

| Файл                           | Роль в проекте           | Почему кандидат  | Что исправить до upscale | Решение |
| ------------------------------ | ------------------------ | ---------------- | ------------------------ | ------- |
| `01-w04-character-messiah`     | хорошо показывает мир;   | атмосфера, образ | сделать low-poly         | ready   |
| `01-w04-character-protagonist` | поддерживает style bible | атмосфера, образ | сделать low-poly         | ready   |
## Environment limitations
- Основной контур работы:
	- локальный ComfyUI + Flux.1 dev fp8;
	- multi-prompt workflow;
	- ControlNet (canny/depth);
	- img2img, inpaint, outpaint;
	- тестирование workflow на собственной машине.
- Что доступно в cloud.comfy.org:
	- базовые workflow ComfyUI;
	- часть Flux workflow;
	- ограниченные image editing возможности;
	- запуск некоторых ControlNet pipeline;
	- limited GPU runtime и cloud-ограничения.
- Что было показано только преподавателем:
	- Flux 2 Klein pipeline;
	- некоторые IP-Adapter workflow;
	- серверные advanced workflow;
	- production-level multi-reference pipeline.
- Что я могу запустить локально:
	...
	- simple upscale.
- Что пока недоступно:
	- полноценный Flux 2 pipeline;
	- часть IP-Adapter workflow;
	- тяжёлые multi-reference workflow;
	- сложные server-only pipeline;
	- некоторые workflow требуют больше VRAM.
- Какой fallback использую:
	- упрощённые workflow;
	- локальные ControlNet pipeline;
	- multi-prompt вместо сложных IP-Adapter setup;
	- image editing через fill/img2img;
	- генерация в пониженном resolution с последующим upscale.